...
import { IGener, genereobj } from "../../data";
...
const GenereDetails = () => {
  ...
  const [singleGenere, setSingeGenere] = useState<IGener| null>(null);
  const [data, setData] = useState<song[] | null>(null);
  const { gnname } = useParams();
  ...
  const fetchSong = async () => {
    ...
    try {
      setIsloading(true);
      const response = genereobj.filter((gn)=> gn.name == gnname)[0]
      setSingeGenere(response)
      const relatedData = songs.filter((song) => song.genere.includes(gnname)); 
  ...
  console.log(singleGenere);
  if (isloading || !singleGenere) {
    return <Loader />;
  }
  ...
  return (
    <SongDetailsWrapper>
      <HeaderContainer>
        <ImageContainer>
          <SongImage alt="song Img" src={singleGenere.image} />
        </ImageContainer>
        <DetailsContainer>
          <p style={{ padding: "5px" }}>song</p>
          <SongTitle>{singleGenere.name}</SongTitle>
        </DetailsContainer>
      </HeaderContainer>
     
      <RelatedSongs
        title={`${singleGenere.name} Songs`}
        data={data}
        isPlaying={isPlaying}
